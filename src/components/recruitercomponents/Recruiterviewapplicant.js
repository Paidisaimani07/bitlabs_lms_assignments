...
const [applicants1, setApplicants1] = useState({
  ...
  const handleDownloadPDF = async () => {
    ...
    const firstElement = borderElements[0];

    if (firstElement) {
        const imageElement = firstElement.querySelector('.imgOne');
        if (imageElement) {
            imageElement.style.width = '100px';
            imageElement.style.height = '100px';
        }

        firstElement.style.position = 'relative'; // Ensure firstElement is the reference for absolute positioning

        const containerDiv = document.createElement('div');
        containerDiv.style.position = 'absolute'; // Absolute positioning within firstElement
        ...
        // Append the containerDiv inside firstElement so it stays within its content
        firstElement.appendChild(containerDiv);

        const textToIncrease = firstElement.querySelectorAll('.textFont');
        textToIncrease.forEach((element, index) => {
            if (index === 0) {
    ...
    const page1 = combinedPdf.addPage([pageWidth, pageHeight]);
    ...
    const copiedPages = await combinedPdf.copyPages(resumePdf, resumePdf.getPageIndices());
    ...
    link.setAttribute("download", `${resume}_bitlabs.pdf`);
        ...
        <div className="themes-container">
          <div className="row">
            <div className="col-lg-12 col-md-12">
              <div className="title-dashboard" style={{marginBottom:'-50px'}}>
                <div className="title-dash flex2">
                  <BackButton /> Applicants
                </div>
                <div className="col-12 d-flex justify-content-center justify-content-md-end" style={{position:'relative',top:'-55px'}}>
